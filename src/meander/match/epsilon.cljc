(ns ^:no-doc meander.match.epsilon
  ...
  (:require [#?(:clj clojure.core :cljs cljs.core) :as clojure]
            ...
            [clojure.zip :as zip]
            [clojure.set :as set]
            #?(:clj [meander.match.check.epsilon :as r.match.check])
            #?(:clj [meander.match.ir.epsilon :as r.ir])
            #?(:clj [meander.matrix.epsilon :as r.matrix])
            #?(:clj [meander.syntax.epsilon :as r.syntax])
            [meander.match.syntax.epsilon :as r.match.syntax]
            [meander.match.runtime.epsilon :as r.match.runtime]
            #?(:clj [meander.util.epsilon :as r.util])
...
(defn analyze-find-args
  ...
  ([match-args env]
   ...
   #?(:clj(let [result (parse-match-args match-args env)]
            (if-some [error (first (get result :errors))]
              (throw error)
              (let [clauses (get result :clauses)
                    contains-cata? (boolean (some :contains-cata? clauses))
...
(defmacro find
  ...
  #?(:clj (let [env (merge (meta &form) &env)
